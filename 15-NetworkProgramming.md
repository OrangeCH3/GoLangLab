...
![Socket示意图](./material/Socket示意图.png)
...
接下来在服务端和客户端分别使用上面定义的`proto包`的`Decode`和`Encode`函数处理数据。
